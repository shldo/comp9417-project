...
from src.train_xrfm import DEFAULT_SAMPLE_SIZE, run_xrfm_experiment


def main() -> None:
    csv_path = RAW_DATA_DIR / DEFAULT_DATASET_NAME
    metrics_path = METRICS_DIR / "xrfm_metrics.json"

    try:
        result = run_xrfm_experiment(
            csv_path=csv_path,
            sample_size=DEFAULT_SAMPLE_SIZE,
            metrics_output_path=metrics_path,
        )
    except ModuleNotFoundError as exc:
        print(f"Cannot run xRFM experiment: {exc}")
        return
    ...
    print(f"xRFM params: {result['xrfm_params']}")
