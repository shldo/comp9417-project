...
from src.train_xrfm import (
    ...
    create_xrfm_classifier,
    evaluate_binary_classifier,
)
from src.timing import measure_training_time
...
JSON_OUTPUT_PATH = METRICS_DIR / "xrfm_tuning_round2_10000.json"
CSV_OUTPUT_PATH = METRICS_DIR / "xrfm_tuning_round2_10000.csv"


ROUND2_CONFIGS = [
    {
        "config_id": "E0",
        "notes": "Anchor E baseline",
    ...
    {
        "config_id": "E2",
        "notes": "Smaller leaves: max_leaf_size=2000",
        "use_one_hot_labels": False,
        "params": {
            "tuning_metric": "auc",
            "max_leaf_size": 2000,
    ...
    {
        "config_id": "E3",
        "notes": "Larger leaves: max_leaf_size=5000",
        "use_one_hot_labels": False,
        "params": {
            "tuning_metric": "auc",
            "max_leaf_size": 5000,
    ...
    {
        "config_id": "E6",
        "notes": "E anchor plus n_tree_iters=1 using one-hot labels workaround",
...
def _prepare_targets(y_train, y_val, use_one_hot_labels: bool):
    """Prepare label arrays for xRFM based on the current config."""
    if not use_one_hot_labels:
        return y_train.to_numpy(), y_val.to_numpy()

    y_train_input = np.eye(2, dtype=np.float32)[y_train.to_numpy()]
    y_val_input = np.eye(2, dtype=np.float32)[y_val.to_numpy()]
    return y_train_input, y_val_input
...
def evaluate_config(config: dict[str, Any], prepared: dict[str, Any]) -> dict[str, Any]:
    ...
    y_train_input, y_val_input = _prepare_targets(
        prepared["y_train"],
        prepared["y_val"],
        config["use_one_hot_labels"],
    )
    ...
    try:
        ...
        model, training_time_seconds = measure_training_time(
            model.fit,
            prepared["X_train"],
            y_train_input,
            prepared["X_val"],
        ...
        return {
            ...
            "use_one_hot_labels": config["use_one_hot_labels"],
            "params": params,
...
def save_results(results: list[dict[str, Any]], prepared: dict[str, Any]) -> None:
    """Save second-round tuning results to JSON and CSV."""
    ensure_project_dirs()
    ...
    json_payload = {
        ...
        "random_state": RANDOM_STATE,
        "failure_diagnosis": (
            "For xrfm 0.4.3, configs with n_tree_iters > 0 require 2D classification targets "
            "with at least two columns in the internal score_tree path. "
            "Round-2 config E6 uses one-hot labels as a workaround."
        ),
...
def main() -> None:
    ...
    print(f"xRFM round-2 tuning on sample_size={SAMPLE_SIZE}")
