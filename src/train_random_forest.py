...
def _raise_if_sklearn_missing() -> None:
    if RandomForestClassifier is None:
        raise ModuleNotFoundError(
            "scikit-learn is not installed in the current environment. "
            "Please install it before running Random Forest training."
        ) from SKLEARN_IMPORT_ERROR


def create_random_forest_classifier(params: dict[str, Any], random_state: int) -> Any:
    """Create a simple RandomForestClassifier for binary classification."""
    _raise_if_sklearn_missing()
...
def train_single_random_forest_model(
    X_train: np.ndarray,
    y_train: pd.Series,
    params: dict[str, Any],
    random_state: int,
) -> tuple[Any, float]:
    """Train one Random Forest model and return the fitted model and elapsed time."""
    model = create_random_forest_classifier(params=params, random_state=random_state)
...
def select_best_random_forest_model(
    X_train: np.ndarray,
    y_train: pd.Series,
    X_val: np.ndarray,
    y_val: pd.Series,
...
def save_random_forest_metrics(metrics_payload: dict[str, Any], output_path: str | Path) -> Path:
    """Save Random Forest metrics to a JSON file."""
    ensure_project_dirs()
...
def run_random_forest_experiment(
    ...
    """
    Run a complete Random Forest experiment with train/validation/test split.
    ...
    best_result = select_best_random_forest_model(
    ...
    output_path = metrics_output_path or (METRICS_DIR / "random_forest_metrics.json")
    saved_path = save_random_forest_metrics(final_payload, output_path)
